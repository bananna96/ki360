...
export const LogoLoop = React.memo<LogoLoopProps>(
	({
		...
		const repeatedLogos = useMemo(
			() => Array.from({ length: 3 }).flatMap(() => logos),
			[logos],
		)
		...
		const logoLists = useMemo(
			() =>
				Array.from({ length: copyCount }, (_, copyIndex) => (
					...
					>
						{repeatedLogos.map((item, itemIndex) =>
		...
		const onWheel = (e: React.WheelEvent<HTMLDivElement>) => {
			...
			if (!el) return
			// Sowohl deltaY (vertikales Scrollen) als auch deltaX (horizontales Scrollen) berücksichtigen
		...
		const handleScroll = () => {
			...
			if (el.scrollLeft < segment * 0.1) {
				el.scrollLeft += segment
			} else if (el.scrollLeft > segment * 1.9) {
				el.scrollLeft -= segment
			}
		}
